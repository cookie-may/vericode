...
export default function VizTreemap({ data, onNodeClick, colorMode }: Props) {
  ...
  useEffect(() => {
    ...
    const root = d3.hierarchy<unknown>(hier)
      .sum(d => d.value ?? 0)
      .sort((a, b) => (b.value ?? 0) - (a.value ?? 0));

    d3.treemap<unknown>().size([w - 20, h - 20]).padding(3).round(true)(root);

    const getColor = (d: unknown) => {
      if (colorMode === 'layer') {
    ...
    // d3.treemap() mutates root in place to add x0/y0/x1/y1 — cast leaves to unknown
    const leaves = root.leaves() as unknown[];
    const cells = g.selectAll<SVGGElement, unknown>('g.tcell')
      .data(leaves)
    ...
    cells.append('rect')
      ...
      .attr('fill', (d: unknown) => getColor(d)).attr('opacity', 0.85)
      .attr('rx', 3).attr('stroke', '#0A0A0C').attr('stroke-width', 1)
      .style('cursor', 'pointer');

    cells.filter((d: unknown) => d.x1 - d.x0 > 45 && d.y1 - d.y0 > 22)
    ...
    svg.on('dblclick.zoom', e => {
      e.preventDefault();
      svg.transition().duration(300).call((zoom as unknown).scaleTo, 1);
    });
